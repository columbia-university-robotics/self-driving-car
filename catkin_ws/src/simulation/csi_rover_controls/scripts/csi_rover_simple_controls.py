...
def csi_robot_OdomCallback(message,cargs):
    ...
    # Reporting
    print('csi_robot_OdomCallback: x=%4.1f,y=%4.1f dist=%4.2f, cmd.v=%4.2f, cmd.w=%4.2f'%(pose[0],pose[1],distance,v,w))

########################################
# Main Script
# Initialize our node
rospy.init_node('csi_robot_control',anonymous=True)
    
# Set waypoint for csi_robot_ to drive to
goal = [0,0]  # Goal
...
rospy.Subscriber('odometry/filtered',nav_msgs.msg.Odometry,csi_robot_OdomCallback, 
                 (cmdpub,cmdmsg,goal))
