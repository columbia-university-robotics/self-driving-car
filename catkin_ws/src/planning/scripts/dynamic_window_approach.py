...
class DWAPlanner:
    def __init__(self, sx, sy, yaw, wx, wy, grid, map_metadata, map_origin) -> None:
        self.x = np.array([sx, sy, yaw, 0.0, 0.0])
        self.wx, self.wy = rotate_waypoints(sx, sy, wx, wy)
        self.goal = np.array([wx[0], wy[0]])
    ...
    def __call__(self):
        ...
        while not rospy.is_shutdown():
            u, predicted_trajectory = dwa_control(
                ...
                config,
                self.goal,
                (self.grid, self.map_metadata, self.map_origin),
            )
            ...
            dist_to_goal = math.hypot(
                self.x[0] - self.goal[0], self.x[1] - self.goal[1]
            )
            if dist_to_goal <= config.robot_radius:
                # Move through waypoints
                self.wx = np.concatenate((self.wx[1:], self.wx[:1]), axis=0)
                self.wy = np.concatenate((self.wy[1:], self.wy[:1]), axis=0)

                # Set new goal
                self.goal = np.array([self.wx[0], self.wy[0]])
...
if __name__ == "__main__":
    ...
    planner = DWAPlanner(sx, sy, yaw, wx, wy, grid, map_metadata, map_origin)
    planner()
